...
def resp_get_logout(user_logged):
    return user_logged.get(reverse('core:logout'))


@pytest.fixture
def resp_post_login(client, user):
    return client.post(
        reverse('core:login'), {'username': user.username, 'password': user.senha}
    )
...
def resp_post_login_error_redirect(client, user):
    return client.post(
        reverse('core:login'), {'username': 'teste_error', 'password': user.senha}
    )
...
def test_logout_redirect(resp_get_logout):
    ...
    assert resp_get_logout.url == reverse('core:login')
...
def test_login_error_redirect(resp_post_login_error_redirect):
    assert resp_post_login_error_redirect.url == reverse('core:login')
